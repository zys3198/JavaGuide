...
| 层级               | 谁在管             | 存什么                                     | 生命周期                             |
| ------------------ | ------------------ | ------------------------------------------ | ------------------------------------ |
| **CLAUDE.md**      | 人工手写或者 /init | 项目规范、编码标准、团队约定               | 永久                                 |
...
整个过程和大脑的记忆机制很像。你的五感一次性接收到的信息就像 Session Memory，海马体和大脑其他对应区域快速存短期记忆和长期记忆（像 Auto Memory），晚上 NREM 睡眠回放给大脑皮层，REM 阶段做整合和选择性遗忘（像 Auto Dream）。虽然 Claude Code 晚上不会梦见电子羊，但它确实在你关掉窗口以后，悄悄把该留的留下来、该忘的忘掉。

## 记忆插件

前面聊的是 Claude Code **内置**的记忆系统。这里列几个社区有四个值得一提的工具：

| 工具                                                                | 定位        | 一句话                                                                                                                    |
| ------------------------------------------------------------------- | ----------- | ------------------------------------------------------------------------------------------------------------------------- |
| [**agentmemory**](https://github.com/rohitg00/agentmemory)          | 功能最全    | 12 个生命周期 hook 全自动捕获；BM25+向量+知识图谱三路融合检索；4 层记忆固化（工作→情景→语义→程序）；零外部依赖，单 SQLite |
| [**claude-mem**](https://github.com/thedotmack/claude-mem)          | 社区最火    | ChromaDB 向量库，6 个 hook 全自动，常驻守护进程带 Web 查看器——代价是 Bun+Python+ChromaDB 的重依赖链                       |
| [**ECC**](https://github.com/affaan-m/ecc)                          | 不止记忆    | Claude Code 整体增强框架；记忆子系统只做跨会话状态保持（session start/compact/end 三阶段），不碰语义长期记忆              |
| [**supermemory**](https://github.com/supermemoryai/supermemory-mcp) | 跨 LLM 通用 | 云端 API，ChatGPT/Claude/Cursor 共用同一份记忆；无需安装，免费——但要信任云端                                              |

如果你希望有更全面的功能、记住更加全面的记忆，你可以尝试一下这些插件。
